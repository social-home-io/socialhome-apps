...
import { createClient, type SocialHomeClient } from "./index.js";
...
describe("RPC calls", () => {
  ...
  it("openSession resolves the session id string", async () => {
    const h = makeHarness();
    active = h.client;
    const p = h.client.federation.openSession("peerB");
    expect(h.posted[0]).toMatchObject({
      method: "app.openSession",
      params: { peer_instance_id: "peerB" },
    });
    h.fromHost({ id: h.posted[0].id, ok: true, result: "sess-123" });
    await expect(p).resolves.toBe("sess-123");
  });

  it("federation.send posts session_id/peer/payload", () => {
    const h = makeHarness();
    active = h.client;
    void h.client.federation.send("sess-1", "peerB", { move: "e4" }).catch(() => {});
    expect(h.posted[0]).toMatchObject({
      method: "app.send",
      params: { session_id: "sess-1", peer_instance_id: "peerB", payload: { move: "e4" } },
    });
  });
